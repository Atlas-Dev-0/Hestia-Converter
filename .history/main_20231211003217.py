...
class ConverterApp(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.converter_thread = None  # Declare converter_thread as an instance variable
    ...
    def convert_file(self):
        ...
        converter_thread = ConverterThread(input_file, output_file, self)
        converter_thread.progress_signal.connect(self.update_progress)
    ...
    def update_progress(self, progress):
        if progress == -1:
            self.log_message(
                f"Error during conversion: {converter_thread.error_message}")
        elif 0 <= progress <= 1:
            progress_percent = int(progress * 100)
            self.log_message(f"Conversion Progress: {progress_percent}%")
